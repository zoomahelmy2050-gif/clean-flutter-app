...
class HybridAuthService with ChangeNotifier {
  ...
  Future<void> _loadLocalData() async {
    try {
      ...
      if (usersJson != null && usersJson.isNotEmpty) {
        try {
          // Skip encrypted data (starts with 'enc1:')
          if (!usersJson.startsWith('enc1:')) {
            final Map<String, dynamic> usersMap = jsonDecode(usersJson);
            _users.clear();
            usersMap.forEach((key, value) => _users[key] = value.toString());
          } else {
            developer.log('Skipping encrypted users data', name: 'HybridAuthService');
          }
        } catch (e) {
          developer.log('Error parsing users data: $e', name: 'HybridAuthService');
          // Clear corrupted data
          await _prefs.remove(_usersKey);
        }
      }
      
      // Load security keys with error handling
      final keysJson = _prefs.getString(_securityKeysKey);
      if (keysJson != null && keysJson.isNotEmpty) {
        try {
          // Skip encrypted data (starts with 'enc1:')
          if (!keysJson.startsWith('enc1:')) {
            final Map<String, dynamic> keysMap = jsonDecode(keysJson);
            _securityKeys.clear();
            keysMap.forEach((key, value) => _securityKeys[key] = value.toString());
          } else {
            developer.log('Skipping encrypted security keys data', name: 'HybridAuthService');
          }
        } catch (e) {
          developer.log('Error parsing security keys data: $e', name: 'HybridAuthService');
          // Clear corrupted data
      ...
      developer.log('Error loading local data: $e', name: 'HybridAuthService');
      // Reset to defaults if there's a general error
      _password = 'password';
      _users.clear();
      _securityKeys.clear();
      _rememberMe = false;
  ...
  Future<List<Map<String, dynamic>>> getAllUsersDetailed() async {
    ...
    for (final entry in _users.entries) {
      usersList.add({
        'email': entry.key,
        'isBlocked': _blockedUsers[entry.key] ?? false,
        'hasMfa': _userMfa[entry.key] ?? false,
        'hasBiometric': _userBiometric[entry.key] ?? false,
        'securityKey': _securityKeys[entry.key],
      });
    }
  ...
  Future<Map<String, dynamic>> register(String email, String password) async {
    ...
    _users[email] = _hashPassword(password, email);
    _securityKeys[email] = _generateSecurityKey();
    await _saveUsers();
    await _saveSecurityKeys();
  ...
  Future<String> resetUserPassword(String email) async {
    ...
    _users[email] = _hashPassword(newPassword, email);
    await _saveUsers();
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    ...
    if (_blockedUsers[email] == true) {
      return {'success': false, 'message': 'User is blocked'};
    }
    
    final hashedPassword = _hashPassword(password, email);
    if (_users[email] != hashedPassword) {
